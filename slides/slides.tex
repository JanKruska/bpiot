...
\newcommand{\green}{\color{green}#1\color{black}}
...
\begin{frame}
	...
	\begin{itemize}
		\item \emph{act2vec}: Analogous to \emph{word2vec} (Does not use activity attributes currently)\temporal<2->{}{\green{Base building block, simple, robust}}{}
		\item \emph{trace2vec}: Utilizes \emph{act2vec} and adds a paragraph vector, analogous to \emph{doc2vec} \temporal<2->{}{\green{Useful for direct trace comparison}}{}
		\item<-3> \emph{log2vec}: Extends \emph{trace2vec} by including an averaging operation, to create an embedding for the whole log \temporal<2->{}{\alert{Too coarse}}{}
		\item<-3> \emph{model2vec}: Embedding of the graph structure of a model by performing random walks \temporal<2->{}{\alert{Complex, not very robust, not useful for log/model comparisons}}{}
	\end{itemize}
\end{frame}
